...
@EActivity(R.layout.view_login)
public class LoginActivity extends AppCompatActivity implements BancoDadosApi
{
	...
	//Usuario passado para tods o sistema
	Usuario usuario;
	...
	void configuracaoInicial()
	{
		...
		//cria o dilogo mas so mostra quando for preciso
	...
	public void logar()
	{
		...
		procuraLoginBancoDados();
	}

    @Click(R.id.btNovoCadastro)
	...
	void logarSistema()
	{
		salvaLogin(edtEmail.getText().toString().trim(), edtSenha.getText().toString());
		vaiParaListaPrincipal();
	}
	
	void procuraLoginBancoDados()
	{
		try
		{
			t.pegaLogin(edtEmail.getText().toString(), edtSenha.getText().toString());
		}
	...
	void vaiParaNovoCadastro()
	{
        NovoCadastroActivity_.intent(this).start();
